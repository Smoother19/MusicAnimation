...
def decode(fn:str):
    if areUaMidi(filename=fn):
        return
    else:
        yh, yp, sr = load(filename=fn)
        X_mag, X_phase = harmSTFT(yharm=yh)
        W, H = nmfdecompose(X_mag,n_composants, seed)

def areUaMidi(filename:str):
...
def harmSTFT(yharm:np.ndarray):
    stft_Harm = librosa.stft(y=yharm, hop_length=hop_length, win_length=win_length, n_fft=n_fft)
    X_mag, X_phase = librosa.magphase(stft_Harm)
    return X_mag, X_phase

def nmfdecompose(X_m:np.ndarray, n_c:int, s:int):
